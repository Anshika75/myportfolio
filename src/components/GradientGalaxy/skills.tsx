    ...
    return (
        <>
            <div className="flex flex-col px-4 lg:px-20 items-center pb-20 mt-4 relative">
                ...
                <p className={`text-[#f0fbff] text-justify lg:text-center opacity-80 ${styles.space} mt-12 text-sm lg:text-2xl`}>Embark on a voyage through the mystical realm of my <span className={`${styles.limegradtext} ${styles.limeglow} font-semibold`}>coding canvas!</span> I am the <span className={`${styles.limegradtext} ${styles.limeglow} font-semibold`}>Script Alchemist</span>, beckoning you to behold the harmonious convergence of skills, where every line is a stroke of brilliance, and every language is a hue in the tapestry of digital enchantment.

                    In this domain where invention waltzes with accuracy, my talents are the arcane elixirs that animate digital fantasies. From <span  className={`${styles.limegradtext} ${styles.limeglow} font-semibold`}>HTML</span> to <span  className={`${styles.limegradtext} ${styles.limeglow} font-semibold`}>Next.js</span>Next.js, each line is a pixel of fervor inscribed onto the parchment of imagination. Let us traverse the binary horizons, transmuting code into a magnum opus and pixels into <span  className={`${styles.limegradtext} ${styles.limeglow} font-semibold`}>indelible adventures</span>! 💻✨ </p>
